...
// MuseToken with Governance.
contract MuseToken is BEP20('Muse Token', 'MUSE') {
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    function safeMuseTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 cakeBal = muse.balanceOf(address(this));
        if (_amount > cakeBal) {
            muse.transfer(_to, cakeBal);
        } else {
