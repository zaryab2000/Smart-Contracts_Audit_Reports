...
// File: contracts/OracleInterface.sol
...
contract GErc20 is Bankable {
     ...
     * @param contractName GToken name
     * @param contractSymbol GToken symbol
     * @param depositTerm Deposit term in seconds
     * @param depositInterest Deposit interest in percents multiplied by 100 (5.67% -> _interest = 567)
     * @param withdrawalFine Fine on deposit in case of withdrawal before expiracy in percents (25%)
     * @param rewardTokenAddress Reward token address
     * @param oracleAddress Exchange rate Oracle address
     * @param underlyingTokenAddress ERC20 token address
     */
    function initialize(
        ...
        uint256 withdrawalFine,
        GEG rewardTokenAddress,
        ...
        setUnderlying(underlyingTokenAddress);
    }
     ...
     * @param amount of ERC20 tokens to deposit. Need to pass amount because user approves GToken contract to transfer
     *  his ERC20 tokens
     * @param isRenewal means should user be allowed to claim interest(true) or not(false) after deposit expiration
     */
    function deposit(uint256 amount, bool isRenewal) external {
        _makeDeposit(amount, isRenewal);
    }
    ...
    function makeWithdrawal(uint256 id) external {
        ...
        require(!_deposit.claimed, "Deposit is already claimed.");
    ...
    function _payout() internal {
        ...
        while (gasleft() > CLAIM_GAS && claimQueue.length() > 0) {
            ...
            require(token.transfer(client, amount), "Transfer failed.");
        }
    }
}
