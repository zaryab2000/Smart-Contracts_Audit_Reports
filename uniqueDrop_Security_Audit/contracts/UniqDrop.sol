...
contract UniqDrop is ERC721Enumerable, IERC2981, Ownable, VRFConsumerBase {
    ...
    */
    ...
    function setBaseURILock() external onlyOwner {
        require(baseURILock == false, "Lock already enabled");
        baseURILock = true;
    }

    //fallback function, throws Warning - ignore it
    ...
    function collectPrize(uint256 _tokenId) external {
        ...
        require(_isWinner(_tokenId) == true, "You dint win"); // CHECK WIN FUNCTION SHOULD BE CALLED INSTEAD OF _isWinner
        require(
    ...
    function getMessageHash( address _tokenOwner, uint _tokenId, string memory _claimersName )
        public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_tokenOwner, _tokenId, _claimersName));
    }
    ...
    function verifySignature( address _tokenOwner, uint _tokenId, string memory _claimersName, bytes memory _signature )
        internal view returns (bool)
    {
        bytes32 messageHash = getMessageHash(_tokenOwner, _tokenId, _claimersName);
    ...
    event Claim(address indexed _claimer, uint256 indexed _tokenId, string _claimersName);
    function burn( uint256 _tokenId, string memory _claimersName, bytes memory _signature )
        external
    {
        require(randomResult>0, "Lottery must be held"); //Comment line for burning tests
        require(ownerOf(_tokenId) == msg.sender, "You need to own this token");
        require(verifySignature(msg.sender, _tokenId, _claimersName, _signature), "Signature is not valid" );
    ...
    */
    ...
    function _getPriceParams() private view returns (uint256[3] memory arr) {
        ...
        if (499 - tSupply > 0)
            return [20000000000000000, 499 - tSupply, 40000000000000000];
        else if (1499 - tSupply > 0)
            return [40000000000000000, 1499 - tSupply, 80000000000000000];
        else if (3499 - tSupply > 0)
            return [80000000000000000, 3499 - tSupply, 160000000000000000];
        else if (7499 - tSupply > 0)
            return [160000000000000000, 7499 - tSupply, 320000000000000000];
        else if (9499 - tSupply > 0)
            return [320000000000000000, 9499 - tSupply, 640000000000000000];
        else if (9899 - tSupply > 0)
            return [640000000000000000, 9899 - tSupply, 1000000000000000000];
        else return [1000000000000000000, 10000 - tSupply, 0];
    }
    ...
    function calculateEthPriceForExactUniqs(uint256 _number)
        ...
        returns (uint256)
    {
        require(totalSupply() < _maxUniqly, "Sale has already ended");
        require(
            (_number + totalSupply()) <= _maxUniqly,
    ...
    function mintUniqly(uint256 numUniqlies) external payable {
        require(msg.value >= _calcEthForUniqs(numUniqlies), "Not enough ether");
        require(
    ...
    function setBaseURI(string memory baseURI) external onlyOwner {
        require(baseURILock == false, "Cant update base URI: Lock enabled");
        BASE_URI = baseURI;
    }
